...
After importing the subpackage, we may start by cloning one of the default library to a local file.

```
import RTModel.templates as tmpl

tmpl.clone_default_library('MyLibrary.txt',2)
```

Here 2 indicates that we are copying the library for 2-peak events.
